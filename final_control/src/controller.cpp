#include <final_control/controller.h>
#include <ros/ros.h>
...
controller::controller(ros::NodeHandle& nh, ros::NodeHandle& nh_private):nh_(nh), nh_private_(nh_private)
{
	...
	front_right_cmd_pub = nh_.advertise<std_msgs::Float64>("/final/front_right_joint_velocity_controller/command",10);
	front_left_cmd_pub = nh_.advertise<std_msgs::Float64>("/final/front_left_joint_velocity_controller/command",10);
	back_right_cmd_pub = nh_.advertise<std_msgs::Float64>("/final/back_right_joint_velocity_controller/command",10);
	back_left_cmd_pub = nh_.advertise<std_msgs::Float64>("/final/back_left_joint_velocity_controller/command",10);
